/*
	Name: unigfcs.h (Universal game functions) v1.3
...
DWORD WINAPI consolesizecheckthread(LPVOID lpvoid)
{
	while (true)
	{
		...
		{
			consoleSizeChanged = getconsolesize();
...
DWORD WINAPI cheatpromptthread(LPVOID lpvoid)
{
	...
	while (true)
	{
		if (showCheatPrompt || consoleSizeChangedCheatPrompt)
		{
			...
			FillConsoleOutputAttribute(GetStdHandle(STD_OUTPUT_HANDLE), backgroundColor + (backgroundColor << 4), length, ORIGIN, &attribsWritten);
...
void copybuffer(const HANDLE *dest, const HANDLE *source)
{
	...
	ReadConsoleOutput(*source, charInfo[0], bufferSize, ORIGIN, &region);
	WriteConsoleOutput(*dest, charInfo[0], bufferSize, ORIGIN, &region);
}
...
void cls()
{
	...
	FillConsoleOutputAttribute(GetStdHandle(STD_OUTPUT_HANDLE), backgroundColor + (backgroundColor << 4), (consoleSize.X + 1) * (consoleSize.Y + 1), ORIGIN, &attribsWritten);
	system("cls");
}
...
COORD getcursorposition()
{
	...
	GetConsoleScreenBufferInfo(GetStdHandle(STD_OUTPUT_HANDLE), &stdOutInfo);
	return stdOutInfo.dwCursorPosition;
}
...
void hcenter(int length)
{
	commonCursorPosition = getcursorposition();
...
void wheelselection(COORD position, int selected, int min, int max, int step)
{
	...
	for (int i = -2; i <= 2; i++)
	{
		setforeground(i == 0 ? White : (i == -1 || i == 1 ? LightGray : DarkGray));
...
void changelog(const char CHANGELOG[][CHANGELOG_COLUMNS], int size)
{
	...
	while (true)
	{
		...
		if (renderChangelog)
		{
			...
			{
				hcenter(CENTERING);
				printf("%s%-*s \n", (CHANGELOG[row + i][0] == '-' ? "  " : ""), CHANGELOG_COLUMNS, CHANGELOG[row + i]);
			}
